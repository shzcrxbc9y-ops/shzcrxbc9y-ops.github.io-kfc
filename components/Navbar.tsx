      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          <Link href="/" className="flex items-center space-x-2 flex-shrink-0">
            <span className="text-xl sm:text-2xl font-bold text-primary-600 dark:text-primary-400">KFC</span>
            <span className="text-gray-600 dark:text-gray-300 hidden sm:inline">Training</span>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-2 lg:space-x-4">
            {user ? (
              <>
                ...
                <div className="flex items-center space-x-2 pl-2 lg:pl-4 border-l border-gray-200 dark:border-gray-700">
                  <User className="w-4 h-4 text-gray-600 dark:text-gray-300 flex-shrink-0" />
                  <span className="text-sm text-gray-700 dark:text-gray-200 hidden lg:inline">{user.firstName} {user.lastName}</span>
                  <span className="text-sm text-gray-700 dark:text-gray-200 lg:hidden">{user.firstName}</span>
                  <button
                    onClick={logout}
                    className="ml-2 p-2 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100 transition-colors"
                    title="Выйти"
                  >
              ...
              <>
                <Link
                  href="/login"
                  className="px-3 lg:px-4 py-2 text-gray-700 dark:text-gray-200 hover:text-primary-600 dark:hover:text-primary-400 transition-colors text-sm lg:text-base"
                >
                  Вход
                </Link>
                <Link
                  href="/register"
                  className="px-3 lg:px-4 py-2 bg-primary-600 dark:bg-primary-500 text-white rounded-lg hover:bg-primary-700 dark:hover:bg-primary-600 transition-colors text-sm lg:text-base"
                >
                  Регистрация
                </Link>
              </>
            ...
            {user ? (
              <div className="space-y-2">
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-gray-700 mt-4">
                  <div className="flex items-center space-x-3 px-4 py-2">
                    ...
                    <span className="text-sm text-gray-700 dark:text-gray-200">{user.firstName} {user.lastName}</span>
                  </div>
                  <button
                    onClick={() => {
                      logout()
              ...
              <div className="space-y-2">
                ...
                <Link
                  href="/register"
                  onClick={() => setMobileMenuOpen(false)}
                  className="block px-4 py-2 bg-primary-600 dark:bg-primary-500 text-white rounded-lg hover:bg-primary-700 dark:hover:bg-primary-600 transition-colors text-center"
                >
                  Регистрация
                </Link>
              </div>
